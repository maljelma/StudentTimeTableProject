document.addEventListener("DOMContentLoaded", () => {
    const VERTICAL_SCROLLABLE_WIDGET = 'vertical-scrollable-widget'
    const VERTICAL_SCROLLABLE_NOTEBOOK = 'vertical-scrollable-notebook';
    const VERTICAL_SCROLLABLE_TAB = 'vertical-scrollable-tab';
    const VERTICAL_SCROLLABLE_INDICATORS = 'vertical-scrollable-indicators'
    // ATTRIBUTES OPTIONS
    ...
    const updateTabIndicator = ({ target }) => {
        ...
        const tabHeight = scrollHeight / TABS_COUNT.value;
        const scrollPosition = parseFloat((scrollTop / tabHeight).toFixed(2));
        //
    ...
    const getCurrentTabIndex = (notebook) => {
        ...
        const scrollTop = notebook.scrollTop;
        const scrollHeight = notebook.scrollHeight
        const tabHeight = scrollHeight / TABS_COUNT.value;
        const scrollPosition = parseFloat((scrollTop / tabHeight).toFixed(2));
        //
    ...
    const updateAllowMouseWheel = (notebook, allow) => {
        if (allow) {
            ...
            notebook.style.overflowY = 'hidden';
        }
    }
    ...
    */
    let widgets = document.querySelectorAll(`div[${VERTICAL_SCROLLABLE_WIDGET}]`);
    widgets.forEach(widget => {
        ...
        * @type {Array<HTMLDivElement>}
        */
        let notebooks = widget.querySelectorAll(`div[${VERTICAL_SCROLLABLE_NOTEBOOK}]`);
        notebooks.forEach(notebook => {
            ...
            notebook.style.overflowY = 'auto';
            notebook.style.scrollSnapType = 'y mandatory';
            ...
            * @type {Array<HTMLDivElement>}
            */
            const tabs = widget.querySelectorAll(`div[${VERTICAL_SCROLLABLE_TAB}]`);
            tabs.forEach(tab => {
            ...
            * @type {HTMLDivElement}
            */
            const indicator = widget.querySelector(`div[${VERTICAL_SCROLLABLE_INDICATORS}]`);
